...
(function() {
    ...
    /**
     * Return Date from string if it contains JSON date format
     *
     * @param {string} value value which need tryParse
     * @return {any} Date if ok or value if string or not JSON-date
     */
    ...
    /**
     * Return result of comparison by Date-values only (without Time)
    ...
    /**
     * Compare dates
     * @return {number} return true if dates is equal (including isNaN == isNaN comparison)
     */
     ...
     * @return {number} return true if Date-values is the same (including isNaN == isNaN comparison)
     */
